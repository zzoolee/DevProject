...
public class PaginationInfoVO<T> {
	...
	// PaginationInfoVO 객체를 만들 때, 한 페이지당 게시글 수와 페이지 블록 수를 원하는 값으로 초기화할 수 있다.
	...
	public String getPagingHTML() {
		...
		// 1-5 범위안에 있는 경우는 Prev가 생겅되지 않는다.
		// 6 범위부터 Prev가 만들어지는 조건이 생성된다.
		StringBuffer html = new StringBuffer();
		html.append("<ul class='pagination pagination-sm m-0 float-right'>");
		
		if(startPage > 1) { // 현재 페이지가 1~5사이면, 시작페이지는 항상 1
